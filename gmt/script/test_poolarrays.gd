...
func test(good, p_seed=null):
	var bytes = PoolByteArray([1,2,3,4,5,6,11,12,13,14,55,255,123])
	...
	var vec2s = PoolVector2Array([
		Vector2(-15, 23),
		Vector2(-35, 27),
		Vector2(256.3, -1)
	])
	var vec3s = PoolVector3Array([
		Vector3(45, 654.3, PI),
		Vector3(45, PI, -2),
		Vector3(-PI, -54, 23.5)
	])
	...
	var ints = PoolIntArray([
		15, 27, 25035345, 456456341, 23546576474, 3466578
	])
	var reals = PoolRealArray([
